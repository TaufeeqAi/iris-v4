...
from fastapi import HTTPException, WebSocket, status # Import WebSocket and status
from agent.db_core.models.user import User
from agent.auth_api.models import UserCreate, TokenData
...
# e.g., JWT_SECRET_KEY="your_base64_encoded_secret_key_here="
# e.g., JWT_REFRESH_SECRET_KEY="your_base64_encoded_refresh_secret_key_here="
...
# NEW: WebSocket token verification dependency
async def get_current_user_ws(websocket: WebSocket) -> str:
    """
    Dependency to get the current authenticated user's username from a WebSocket connection.
    Expects the token in the 'token' query parameter.
    Closes the WebSocket with a policy violation if authentication fails.
    """
    ...
    try:
        ...
        username: str = payload.get("sub")
        if username is None:
            logger.warning("WebSocket authentication failed: Token missing 'sub' claim.")
            await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Invalid token (missing user ID)")
            raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token")

        # Optional: Verify token type if you only want access tokens for WS
        ...
        logger.info("WebSocket authenticated user: %s", username)
        return username
    except JWTError as e:
        logger.error("WebSocket authentication failed: JWTError - %s", e)
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Invalid authentication token")
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid authentication token")
    except Exception as e:
        logger.error("WebSocket authentication failed: Unexpected error - %s", e)
