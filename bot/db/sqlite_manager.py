...
class SQLiteManager:
    ...
    def _ensure_schema(self):
        ...
        try:
            ...
            # IMPORTANT: Changed persona and llm_model_name to be nullable (removed NOT NULL)
            ...
            # Add 'llm_model_name' column if it doesn't exist (and ensure it's nullable)
    ...
    async def save_agent_config(self, agent_config: AgentConfig) -> str:
        ...
        def _save():
            ...
            try:
                ...
                # Serialize complex fields to JSON strings, handling None correctly
                secrets_json = agent_config.settings.secrets.model_dump_json(exclude_none=True) if agent_config.settings.secrets else "{}"
                # Ensure lists/dicts default to empty JSON representations if None
                ...
                """, (
                    ...
                    agent_config.settings.model, # This will be an empty string if blank from frontend, now nullable in DB
                    agent_config.settings.temperature,
                    agent_config.settings.maxTokens, 
                    agent_config.system, # Save agent_config.system to persona column
                    bio_json, 
                    lore_json, 
                    knowledge_json, 
            ...
            except sqlite3.Error as e: # Catch generic sqlite3.Error for more robust handling
        ...
        def _update():
            ...
            try:
                ...
                """, (
                    ...
                    agent_config.settings.temperature,
                    agent_config.settings.maxTokens,
                    agent_config.system, # Update persona column with agent_config.system
                    bio_json,
                    lore_json,
                    knowledge_json,
                    message_examples_json,
    ...
    async def get_agent_config(self, agent_id: str) -> Optional[AgentConfig]:
        ...
        def _get():
            ...
            try:
                ...
                row = cursor.fetchone()
                if row:
                    return self._load_agent_config_from_row(row, cursor.description) # Pass cursor.description
                return None
            except Exception as e:
    ...
    async def get_all_agent_configs(self) -> List[AgentConfig]:
        ...
        def _get_all():
            ...
            try:
                ...
                for row in rows:
                    try:
                        configs.append(self._load_agent_config_from_row(row, cursor.description)) # Pass cursor.description
                    except Exception as e:
    ...
    def _load_agent_config_from_row(self, row: Tuple, cursor_description: List[Tuple]) -> AgentConfig:
        ...
        def safe_json_load(json_str: Optional[str], default_val: Any):
            if json_str is None or (isinstance(json_str, str) and not json_str.strip()): # Handles None and empty strings
                return default_val
            try:
        ...
        llm_model_name = row_dict.get('llm_model_name') # This is a plain string, NOT JSON
        ...
        settings_instance = Settings(
            model=llm_model_name, # This is now directly the string from DB
